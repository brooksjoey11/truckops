...
All prompts below are examples only. They are stored as separate files in `agents/orchestrator/prompts/` and can be customized without modifying the Orchestrator's core logic.
...
To customize any prompt, edit the corresponding file in `agents/orchestrator/prompts/` or create a new prompt file. The Orchestrator reads prompts from that directory at runtime.
