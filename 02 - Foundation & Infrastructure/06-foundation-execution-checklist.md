...
The model provider API key is stored under the name `model_provider_api_key`. The `MODEL_PROVIDER_NAME` and `MODEL_PROVIDER_ENDPOINT` are environment variables, not secrets.
...
## Phase 7 — TruckOps Initialization
...
## Phase 8 — Orchestrator Readiness

| Step | Action | Verification | Status |
|---|---|---|---|
| 8.1 | Create Orchestrator workspace directory | Directory exists | Not Started |
| 8.2 | Add Orchestrator manifest | `agent.yaml` exists | Not Started |
...
## Phase 9 — Foundation Completion Criteria
