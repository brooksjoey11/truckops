...
| `MODEL_PROVIDER_NAME` | Yes | `anthropic`, `local`, etc. | Model provider label |
...
`MODEL_PROVIDER_NAME` and `MODEL_PROVIDER_ENDPOINT` are provider-neutral. No vendor-specific variable names are used.
...
See `tools/initialize_workspace.sh` for the full initialization script.
...
See `tools/health_check.sh` for the full health check script.
