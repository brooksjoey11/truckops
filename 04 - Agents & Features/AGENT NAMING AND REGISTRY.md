...
The registry is extensible. Add new agents by creating a properly-formed agent manifest file and adding an entry to `config/agents.yaml`. No core code changes are required.
...
To register a new agent: create an agent manifest at `agents/{canonical_id}/agent.yaml` and add an entry to `config/agents.yaml`. The Orchestrator will register the agent on next startup.
