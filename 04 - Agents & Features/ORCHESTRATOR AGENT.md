...
The template files for `agents/orchestrator/` are maintained in this repository under `agents/orchestrator/` and deployed to `$TRUCKOPS_HOME/agents/orchestrator/` during foundation setup.

---

## 6. Agent Manifest

See `agents/orchestrator/agent.yaml` in this repository for the complete machine-readable manifest.
...
No specific model provider, vendor, or API endpoint is hard-coded. All model configuration is injected via environment variables.

---

## 7. Core Skills

All skill definitions are stored as YAML files in `agents/orchestrator/skills/`. They are not hard-coded in the agent logic.
...
The agent registry is extensible. A new specialist agent is registered by placing a properly formed `agent.yaml` manifest in `$TRUCKOPS_HOME/agents/{canonical_id}/` and registering it via `config/agents.yaml`. No core Orchestrator code needs to be modified.
...
Workflow definitions are stored as YAML files in `agents/orchestrator/workflows/`. They are not hard-coded in the agent logic.
...
The MVP acceptance test is stored as a separate configurable file: `agents/orchestrator/tests/mvp-acceptance-test.yaml`.

This allows the user to adjust test criteria, input, and expected output without modifying the Orchestrator's core logic.

See `agents/orchestrator/tests/mvp-acceptance-test.yaml` for the current test definition.
...
*This is the complete Orchestrator Agent definition page, ready for implementation.*
